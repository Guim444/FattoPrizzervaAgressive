...
https://assetstore.unity.com/packages/slug/321005#reviews
...
Version 34.4
- Ignore: added "Ignore All Children" option to also exclude child renderers from other Highlight Effects (useful for character roots that have no renderer of their own)
- [Fix] Fixed error when highlighting particle systems with outline or glow quality other than Highest

Version 34.3
- Target FX: new Off-Screen Indicator option, which keeps the marker visible at the screen edges when the object is off-screen

Version 34.2.5
- [Fix] Object Name Filter is no longer applied to objects highlighted via scripting (Include set to Scripting)
- [Fix] Highlight Effect Blocker: fixed blocker incorrectly cancelling outline/glow when the camera is inside the blocker object (e.g. first-person view)
- [Fix] Highlight Effect Blocker: hidden renderers (shadows-only mode or rendering forced off) no longer block outline/glow
- [Fix] Outline/glow masking and blockers were ignored on Android when using Vulkan (stencil contents were lost between render passes)
- [Fix] Focus: the focus effect no longer shows through objects in front of the highlighted object when using Around Object Shape contour or on-top visibility
- [Fix] Highlight Effect Blocker: renderers on layers not rendered by the current camera no longer block outline/glow (new Respect Camera Culling Mask option, enabled by default)
- [Fix] Highlight Effect Blocker: blockers could be ignored on Android Vulkan when outline/glow quality is not Highest
