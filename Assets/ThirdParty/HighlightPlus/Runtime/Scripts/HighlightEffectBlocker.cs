...
namespace HighlightPlus {
    ...
    public class HighlightEffectBlocker : MonoBehaviour {
        ...
        [Tooltip("Ignore renderers on layers not rendered by the current camera (e.g. a first-person body hidden through the camera culling mask)")]
        public bool respectCameraCullingMask = true;
        ...
        public void BuildCommandBuffer (CommandBuffer cmd, Material mat, int cameraCullingMask) {
            if (renderers == null) return;
            int renderersCount = renderers.Count;
            for (int k=0;k<renderersCount;k++) {
                Renderer r = renderers[k];
                if (r == null || !r.enabled || r.forceRenderingOff || !r.gameObject.activeInHierarchy) continue;
                if (r.shadowCastingMode == ShadowCastingMode.ShadowsOnly) continue;
                if (respectCameraCullingMask && ((1 << r.gameObject.layer) & cameraCullingMask) == 0) continue;
