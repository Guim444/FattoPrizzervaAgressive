...
namespace HighlightPlus {
    ...
    public partial class HighlightEffect : MonoBehaviour {
        ...
        /// <summary>
        /// This object won't be highlighted.
        /// </summary>
        [Tooltip("This object won't be highlighted.")]
        public bool ignore;

        /// <summary>
        /// While Ignore is enabled, this object's children are also excluded from other Highlight Effects.
        /// </summary>
        [Tooltip("While Ignore is enabled, this object's children are also excluded from other Highlight Effects.")]
        public bool ignoreAllChildren;
        ...
        [Tooltip("Keeps the target FX visible at the screen edges when the object is off-screen")]
        public bool targetFXOffScreenIndicator;
        [Tooltip("Optional texture for the off-screen indicator, like an arrow. If not set, the target FX texture is used.")]
        public Texture2D targetFXOffScreenIndicatorTexture;
        [Tooltip("Distance from the screen edges for the clamped target FX (normalized)")]
        [Range(0, 0.3f)]
        public float targetFXScreenMargin = 0.05f;
        [Tooltip("While clamped to the screen edges, the target FX points towards the object position")]
        public bool targetFXPointToTarget = true;
        [Tooltip("Size of the clamped target FX as a fraction of the screen height")]
        [Range(0.01f, 0.5f)]
        public float targetFXScreenSize = 0.06f;
        ...
        void Init () {
            ...
            usesVulkan = SystemInfo.graphicsDeviceType == GraphicsDeviceType.Vulkan;
        ...
        void OnValidate () {
            ...
            targetFXScreenMargin = Mathf.Clamp(targetFXScreenMargin, 0f, 0.3f);
            targetFXScreenSize = Mathf.Clamp(targetFXScreenSize, 0.01f, 0.5f);
        ...
        void BuildCommandBuffer (Camera cam, bool clearStencil) {
            ...
            cameraCullingMask = cam.cullingMask;
            ...
            if (clearStencil) {
                ...
                HighlightPlusRenderPassFeature.RenderBlockers(cbHighlight, cameraCullingMask);
            ...
            // Vulkan can discard stencil contents across render target switches; re-stamp blockers for this effect (compose path re-stamps on its own)
            if (usesVulkan && !clearStencil) {
                HighlightPlusRenderPassFeature.RenderBlockers(cbHighlight, cameraCullingMask);
            }
            ...
            if (maskRequired) {
                // Focus exclusion (bit 8) must only cover visible pixels; if the mask renders on top, stamp bit 8 separately with the depth-tested pass
                bool separateFocusMask = focus > 0 && renderMaskOnTop;
                if (focus > 0 && !separateFocusMask) {
                ...
                if (separateFocusMask) {
                    cbHighlight.SetGlobalInt(ShaderParams.MaskStencilRef, 8);
                    cbHighlight.SetGlobalInt(ShaderParams.MaskWriteMask, 8);
                    for (int k = 0; k < rmsCount; k++) {
                        if (rms[k].render) {
                            RenderSeeThroughMask(k, rms[k].mesh);
                        }
                    }
                    cbHighlight.SetGlobalInt(ShaderParams.MaskStencilRef, 10);
                    cbHighlight.SetGlobalInt(ShaderParams.MaskWriteMask, 10);
                }
            }
            ...
            // Off-screen clamped target FX
            bool renderClampedTarget = targetFXOffScreenIndicator && ComputeClampedTargetState(cam, now);
            if (clampedMatStateApplied && !renderClampedTarget && fxMatTarget != null) {
                fxMatTarget.SetInt(ShaderParams.ZTest, GetZTestValue(targetFXVisibility));
                fxMatTarget.mainTexture = targetFXTexture;
                clampedMatStateApplied = false;
            }
            if (targetFXOffScreenIndicator && !somePartVisible) {
                if (renderClampedTarget) {
                    RenderClampedTargetFX(cam, now, fadeGroup);
                }
                return;
            }
            ...
            for (int k = 0; k < rmsCount; k++) {
                    ...
                    for (int l = 0; l < subMeshCount; l++) {
                        ...
                        if (usesOutlineNonHQ) {
                            ...
                            if (outlineColorStyle == ColorStyle.Gradient) {
                                ...
                                Bounds bounds = outlineGradientInLocalSpace ? (mesh != null ? mesh.bounds : rms[k].renderer.localBounds) : rms[k].renderer.bounds;
                            ...
                            if (outlineQuality.UsesMultipleOffsets()) {
                                ...
                                cbHighlight.SetGlobalColor(ShaderParams.OutlineColor, outlineColor);
                                cbHighlight.SetGlobalVector(ShaderParams.OutlineDirection, Vector4.zero);
                                if (rms[k].isSkinnedMesh || rms[k].isParticleSystem) {
                                    cbHighlight.DrawRenderer(rms[k].renderer, rms[k].fxMatOutline[l], l, 0);
                                }
                                else {
                ...
                // Target
                if (targetFX && !renderClampedTarget) {
                    float fadeOut = 1f;
        ...
        Vector3 clampedTargetVP;
        Vector3 clampedTargetFadePosition;
        bool clampedMatStateApplied;

        internal bool targetFXOffScreenIndicatorActive {
            get {
                if (!targetFX || !targetFXOffScreenIndicator) return false;
                // While fading, the pass must keep running so the fade state machine completes and end events fire
                if (fading != FadingState.NoFading) return true;
                if (targetFXStayDuration > 0 && Application.isPlaying && (GetTime() - targetFXStartTime) >= targetFXStayDuration + 1f) return false;
                return true;
            }
        }

        // Off-screen clamped target FX
        bool ComputeClampedTargetState (Camera cam, float now) {
            if (!targetFXOffScreenIndicatorActive || !_highlighted || fxMatTarget == null || quadMesh == null) return false;

            int anchor = -1;
            int camCullingMask = cam.cullingMask;
            for (int k = 0; k < rmsCount; k++) {
                if (rms[k].render) { anchor = k; break; }
            }
            if (anchor < 0) {
                for (int k = 0; k < rmsCount; k++) {
                    Transform t = rms[k].transform;
                    Renderer r = rms[k].renderer;
                    if (t == null || r == null) continue;
                    if (rms[k].mesh == null && !rms[k].isParticleSystem) continue;
                    if (!r.enabled || r.forceRenderingOff) continue;
                    if (!t.gameObject.activeInHierarchy) continue;
                    if (!ignoreObjectVisibility && ((1 << t.gameObject.layer) & camCullingMask) == 0) continue;
                    anchor = k;
                    break;
                }
            }
            if (anchor < 0) return false;
            clampedTargetFadePosition = rms[anchor].transform.position;

            Vector3 point;
            if (targetFxCenterOnHitPosition && currentHitTarget != null) {
                point = currentHitTarget.TransformPoint(currentHitLocalPosition);
            }
            else if (targetFXCenter != null) {
                point = targetFXCenter.position;
            }
            else if (targetFXUseEnclosingBounds) {
                // Own union: ComputeEnclosingBounds() excludes non-rendering combined entries, which is exactly the off-screen case
                Bounds eb = new Bounds();
                bool found = false;
                for (int k = 0; k < rmsCount; k++) {
                    Renderer r = rms[k].renderer;
                    if (r == null) continue;
                ...
                point = rms[anchor].renderer.bounds.center;
            }
            point += targetFXOffset;

            Vector3 vp = cam.WorldToViewportPoint(point, Camera.MonoOrStereoscopicEye.Mono);
            if (float.IsNaN(vp.x) || float.IsInfinity(vp.x) || float.IsNaN(vp.y) || float.IsInfinity(vp.y) || float.IsNaN(vp.z) || float.IsInfinity(vp.z)) return false;
            clampedTargetVP = vp;
            return vp.z <= 0 || vp.x < 0 || vp.x > 1 || vp.y < 0 || vp.y > 1;
        }

        void RenderClampedTargetFX (Camera cam, float now, float fadeGroup) {

            // Stay duration window
            float fadeOut = 1f;
            if (targetFXStayDuration > 0 && Application.isPlaying) {
                fadeOut = now - targetFXStartTime;
                if (fadeOut >= targetFXStayDuration) {
            ...
            float transitionMul;
            if (Mathf.Abs(targetFXEndScale) > 0.0001f) {
                transitionMul = Mathf.Lerp(targetFXInitialScale, targetFXEndScale, scaleT) / targetFXEndScale;
            }
            else {
                transitionMul = Mathf.Lerp(Mathf.Max(Mathf.Abs(targetFXInitialScale), 1f), 1f, scaleT);
            }

            // Marker size capped so it always fits inside the margins
            const float EPSILON = 0.001f;
            float margin = Mathf.Clamp(targetFXScreenMargin, 0f, 0.3f);
            float screenSize = Mathf.Clamp(targetFXScreenSize, 0.01f, 0.5f);
            float currentScreenSize = screenSize * Mathf.Abs(transitionMul);
            float aspect = cam.aspect;
            float maxFit = 1f - 2f * (margin + EPSILON);
            float displayedSize = Mathf.Clamp(currentScreenSize, EPSILON, Mathf.Min(aspect * maxFit, maxFit));

            // Radial projection onto the inset rect border
            Vector3 vp = clampedTargetVP;
            float px = vp.x - 0.5f, py = vp.y - 0.5f;
            if (vp.z <= 0 && !cam.orthographic) {
                // Perspective projection mirrors points behind the camera; orthographic does not
                px = -px;
                py = -py;
            }
            float ex = Mathf.Max(0.5f - margin - displayedSize / (2f * aspect), EPSILON);
            float ey = Mathf.Max(0.5f - margin - displayedSize * 0.5f, EPSILON);
            ...
            float t = Mathf.Max(ax / ex, ay / ey);
            float edgeX = px / t, edgeY = py / t;
            if (float.IsNaN(edgeX) || float.IsInfinity(edgeX) || float.IsNaN(edgeY) || float.IsInfinity(edgeY)) {
                edgeX = 0;
                edgeY = -ey;
            }

            // Reproject at a fixed camera distance (mono projection so both VR eyes get the same world position)
            float nearClip = cam.nearClipPlane;
            float dist = VRCheck.isVrRunning ? 2f : nearClip + 1f;
            dist = Mathf.Clamp(dist, nearClip + 0.01f, Mathf.Max(cam.farClipPlane - 0.01f, nearClip + 0.011f));
            Vector3 edgeVP = new Vector3(edgeX + 0.5f, edgeY + 0.5f, dist);
            Vector3 center = cam.ViewportToWorldPoint(edgeVP, Camera.MonoOrStereoscopicEye.Mono);
            Vector3 upSample = cam.ViewportToWorldPoint(new Vector3(edgeVP.x, edgeVP.y + displayedSize, dist), Camera.MonoOrStereoscopicEye.Mono);
            float worldHeight = (upSample - center).magnitude;
            if (worldHeight <= 0 || float.IsNaN(worldHeight) || float.IsInfinity(worldHeight)) return;
            Vector3 scale = new Vector3(worldHeight, worldHeight, worldHeight);

            Quaternion rotation;
            if (targetFXPointToTarget) {
                float ang = Mathf.Atan2(py, px * aspect) * Mathf.Rad2Deg;
                rotation = cam.transform.rotation * Quaternion.Euler(0, 0, ang - 90f);
            }
            else {
                rotation = Quaternion.LookRotation(cam.transform.forward);
                rotation *= Quaternion.Euler(0, 0, targetFXRotationAngle + time * targetFXRotationSpeed);
            ...
            Matrix4x4 m = Matrix4x4.TRS(center, rotation, scale);
            float fade = fadeGroup;
            if (cameraDistanceFade) {
                // Same reference point as the on-screen path so alpha is continuous when crossing the edge
                fade *= ComputeCameraDistanceFade(clampedTargetFadePosition, cam.transform);
            }
            Color color = targetFXColor;
            color.a *= fade * fadeOut * normalizedTime;
            fxMatTarget.color = color;
            fxMatTarget.SetInt(ShaderParams.ZTest, (int)CompareFunction.Always);
            fxMatTarget.mainTexture = targetFXOffScreenIndicatorTexture != null ? targetFXOffScreenIndicatorTexture : targetFXTexture;
            clampedMatStateApplied = true;
            cbHighlight.DrawMesh(quadMesh, m, fxMatTarget, 0, 1);
        }
        ...
        void ComposeSmoothBlend (Visibility smoothGlowVisibility, Visibility smoothOutlineVisibility) {
            ...
            cbHighlight.SetRenderTarget(colorAttachmentBuffer, RenderBufferLoadAction.Load, RenderBufferStoreAction.Store, useDepthRenderBuffer ? colorAttachmentBuffer : depthAttachmentBuffer, RenderBufferLoadAction.Load, RenderBufferStoreAction.Store);

            // Vulkan can discard stencil contents across render target switches; re-stamp mask (bit 2 only, compose does not read the focus bit) and blockers
            if (usesVulkan) {
                if (maskRequired) {
                    cbHighlight.SetGlobalInt(ShaderParams.MaskStencilRef, 2);
                    cbHighlight.SetGlobalInt(ShaderParams.MaskWriteMask, 2);
        ...
        void SetupMaterial (IList<Renderer> rr) {
            ...
            bool useNameFilter = effectGroup != TargetOptions.OnlyThisObject && effectGroup != TargetOptions.Scripting && !string.IsNullOrEmpty(effectNameFilter);
        ...
        bool ShouldExcludeRenderer (Renderer renderer) {
            ...
            if (current.TryGetComponent(out HighlightEffect selfEffect) && selfEffect.enabled && (selfEffect.ignore || excludeObjectsWithHighlightEffect)) {
                return true;
            }

            current = current.parent;
            while (current != null && current != target) {
                if (current.TryGetComponent(out HighlightEffect otherEffect) && otherEffect.enabled && otherEffect.ignore && otherEffect.ignoreAllChildren) {
                    return true;
                }
                current = current.parent;
            }
        ...
        void CombineMeshes () {
            ...
            rms[first].subMeshCount = combinedMesh.subMeshCount;
        ...
        /// <summary>
        /// True when this effect's current settings would sample _CameraDepthTexture if rendered.
        /// Camera eligibility is checked by the render feature before calling this.
        /// </summary>
        internal bool RequiresCameraDepthTextureContent () {
            bool isHighlighted = highlighted;

            // HighlightSolidColor.shader (Highest Quality outline/glow) samples _CameraDepthTexture when:
            //   HP_DEPTHCLIP / HP_DEPTHCLIP_INV  -> set when an outline/glow Highest effect has visibility != AlwaysOnTop
            //   HP_ALL_EDGES                     -> set when outlineEdgeMode == Any (affects the shared SolidColor material)
            if (isHighlighted) {
                bool outlineHighest = outline > 0 && outlineQuality == QualityLevel.Highest;
                bool glowHighest = glow > 0 && glowQuality == QualityLevel.Highest;
                if (outlineHighest || glowHighest) {
                    if (outlineEdgeMode == OutlineEdgeMode.Any) {
                        return true;
                    }
                    if (outlineHighest && outlineVisibility != Visibility.AlwaysOnTop) {
                        return true;
                    }
                    if (glowHighest && glowVisibility != Visibility.AlwaysOnTop) {
                        return true;
                    }
                }
            }

            // HighlightSeeThrough.shader / HighlightSeeThroughBorder.shader sample _CameraDepthTexture
            // when HP_DEPTH_OFFSET is enabled, i.e. seeThroughDepthOffset > 0 || seeThroughMaxDepth > 0
            // AND see-through is currently active.
            bool usesSeeThroughNow = seeThroughIntensity > 0
                && (seeThrough == SeeThroughMode.AlwaysWhenOccluded
                    || (seeThrough == SeeThroughMode.WhenHighlighted && isHighlighted));
            if (usesSeeThroughNow && (seeThroughDepthOffset > 0 || seeThroughMaxDepth > 0)) {
                return true;
            }

            // HighlightTarget.shader "Target FX Decal" pass unconditionally samples scene depth,
            // and is the pass drawn only when align-to-ground is enabled.
            if (targetFX && targetFXAlignToGround) {
                return true;
            }
