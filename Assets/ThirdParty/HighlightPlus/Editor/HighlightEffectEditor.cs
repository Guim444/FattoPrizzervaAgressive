...
#pragma warning disable 0618
        ...
        SerializedProperty ignore, ignoreAllChildren, effectGroup, effectGroupLayer, effectNameFilter, effectNameUseRegEx, excludeObjectsWithHighlightEffect, effectTarget, combineMeshes, alphaCutOff, alphaCutOffTextureName, cullBackFaces, padding, normalsOption;
        ...
        SerializedProperty targetFXOffScreenIndicator, targetFXOffScreenIndicatorTexture, targetFXScreenMargin, targetFXPointToTarget, targetFXScreenSize;
        ...
        void OnEnable () {
            ...
            ignoreAllChildren = serializedObject.FindProperty("ignoreAllChildren");
            ...
            targetFXOffScreenIndicator = serializedObject.FindProperty("targetFXOffScreenIndicator");
            targetFXOffScreenIndicatorTexture = serializedObject.FindProperty("targetFXOffScreenIndicatorTexture");
            targetFXScreenMargin = serializedObject.FindProperty("targetFXScreenMargin");
            targetFXPointToTarget = serializedObject.FindProperty("targetFXPointToTarget");
            targetFXScreenSize = serializedObject.FindProperty("targetFXScreenSize");
        ...
        public override void OnInspectorGUI () {
            ...
            if (expandHighlightOptions) {
                ...
                if (!isManager) {
                    ...
                    if (ignore.boolValue) {
                        EditorGUI.indentLevel++;
                        EditorGUILayout.PropertyField(ignoreAllChildren, new GUIContent("Ignore All Children", "Children are also excluded from other Highlight Effects. Call Refresh() on the including effect if you change this at runtime."));
                        EditorGUI.indentLevel--;
                    }
                }
                if (!ignore.boolValue) {
            ...
            if (!ignore.boolValue) {
                ...
                if (expandTarget) {
                    ...
                    if (targetFX.boolValue) {
                        ...
                        EditorGUILayout.PropertyField(targetFXOffScreenIndicator, new GUIContent("Off-Screen Indicator", "Keeps the target FX visible at the screen edges when the object is off-screen."));
                        if (targetFXOffScreenIndicator.boolValue) {
                            EditorGUI.indentLevel++;
                            EditorGUILayout.PropertyField(targetFXOffScreenIndicatorTexture, new GUIContent("Indicator Texture", "Optional texture for the off-screen indicator, like an arrow. If not set, the target FX texture is used."));
                            EditorGUILayout.PropertyField(targetFXScreenMargin, new GUIContent("Screen Margin", "Distance from the screen edges (normalized)."));
                            EditorGUILayout.PropertyField(targetFXScreenSize, new GUIContent("Screen Size", "Size of the clamped target FX as a fraction of the screen height."));
                            EditorGUILayout.PropertyField(targetFXPointToTarget, new GUIContent("Point To Target", "Rotates the target FX so it points towards the object position. Use a directional texture like target_arrow."));
                            EditorGUI.indentLevel--;
                        }
                    }
                    EditorGUI.indentLevel--;
